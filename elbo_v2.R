...
diff_tau2 <- function(K, p, shape_lambda_q, rate_q, chi_q, psi_q){
  ...
  res <-  K*sum(E_log_lambda2) - 0.5*sum(sapply(1:p, function(j){E_lambda2[j]*sum(E_tau2[ids[[j]]])})) - sum(0.25*(log(chi_q) - log(as.numeric(psi_q))) - log(sapply(1:(K*p), function(kj){2*BesselK(sqrt(chi_q[kj] * psi_q[kj]), 0.5)}))) + 0.5*sum(E_log_tau2) + 0.5*sum(as.numeric(psi_q)*E_tau2 + chi_q*E_eta)
  #cat("diff_tau2_i :", res, "\n")
  return(res)
}
...
diff_theta <- function(a_q, b_q, a0, b0){
  ...
  sum(sapply(1:p, function(j){(a0 - a_q[j])*(digamma(a_q[j]) - digamma(a_q[j] + b_q[j])) + (b0 - b_q[j])*(digamma(b_q[j]) - digamma(a_q[j] + b_q[j])) + log(gamma(a_q[j])) + log(gamma(b_q[j])) - log(gamma(a_q[j] + b_q[j]))}))
