...
diff_b <- function(K, p, delta1_q, delta2_q, chi_q, psi_q, mu_b_q, Sigma_b_q){
  ...
  res <- -(K*p/2)*(log(delta2_q) - (log(delta1_q) - 1/(2*delta1_q))) -0.5*sum(E_log_tau2) -0.5*E_inv_sigma2*sum((diag(Sigma_b_q) + mu_b_q^2)*E_eta) + 0.5*log(det(Sigma_b_q)) + 0.5*K*p
...
diff_sigma2 <- function(delta1_q, delta2_q, delta2_0, delta1_0){
  ...
  #res <-  - delta1_q*log(delta2_q)  + (delta1_q - delta1_0)*(log(delta2_q) - digamma(delta1_q)) + (delta2_q - delta2_0)*E_inv_sigma2
